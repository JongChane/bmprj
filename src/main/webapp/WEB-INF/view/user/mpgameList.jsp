					...
					<c:forEach var="g" items="${gmuser}" varStatus="stat">
						<tr>
							...
							<td><c:if test="${param.user_id == g.key.user_id}">
									<button type="button" class="btn btn-success btn-sm"data-bs-toggle="modal" data-bs-target="#exampleModal${stat.index }">삭제</button>
									<!-- Modal -->
									<div class="modal fade" id="exampleModal${stat.index }" tabindex="-1"
										...
										<tr>
											...
											<c:if test="${param.user_id != g.key.user_id}">
												<c:if test="${param.user_id == gm.user_id}">
													<button type="button" class="btn btn-primary"data-bs-toggle="modal" data-bs-target="#exampleModal2${stat.index }${st.index}">매치 나가기</button>
												</c:if>
												</c:if>
												<!-- Modal 2 -->
													<div class="modal fade" id="exampleModal2${stat.index }${st.index}" tabindex="-1"
														aria-labelledby="exampleModalLabel" aria-hidden="true">
														<div class="modal-dialog">
															<div class="modal-content">
																<div class="modal-header">
																...
																<div class="modal-footer">
																	<button type="button" class="btn btn-secondary"
																		data-bs-dismiss="modal">닫기</button>
																	<a href="mpudelete?gmnum=${g.key.game_num}&user_id=${sessionScope.loginUser.user_id}"><button
