...
<nav style="width:200px;" id="mySidebar">
  ...
  <div class="w3-bar-block">
    ...
    <a href="${path}/game/gameList?user_id=${sessionScope.login}" 
    class="w3-bar-item w3-button w3-padding <c:if test='${url=="user" }'>w3-blue</c:if>">
    &nbsp; · 매칭 내역</a>
    <a href="${path}/reservation/reserveList?user_id=${sessionScope.login}" 
    class="w3-bar-item w3-button w3-padding <c:if test='${url=="user" }'>w3-blue</c:if>">
    &nbsp; · 예약 내역</a>
    <a href="${path}/board/boardList?user_id=${sessionScope.login}" 
    class="w3-bar-item w3-button w3-padding <c:if test='${url=="user" }'>w3-blue</c:if>">
    &nbsp; · 건의답변 내역</a>
    <hr>
